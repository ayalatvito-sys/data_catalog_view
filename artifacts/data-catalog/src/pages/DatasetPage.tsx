...
import {
  ArrowBack, TableChart, AccountTree, Circle,
  AttachMoney, LocationOn, Security
} from '@mui/icons-material';
...
export default function DatasetPage() {
  ...
  const [activeProject, setActiveProject] = useState<string>('כללי'); // State חדש
  //עכשיו
  const [activeSystem, setActiveSystem] = useState<string>('הכל'); // <-- ה-State החדש
  ...
  // // שליפת כל שמות הפרויקטים הייחודיים (אם אין, יקבל "כללי")
  // const uniqueProjects = [...new Set(tables.map(t => t.project_name || 'כללי'))];
  
  // // ברגע שהטבלאות נטענות, נוודא שהפרויקט הפעיל חוקי (או נבחר את הראשון)
  // useEffect(() => {
  ...
  // // סינון הטבלאות לפי הפרויקט שנבחר
  // const displayedTables = tables.filter(t => (t.project_name || 'כללי') === activeProject);

  // שליפת כל שמות הפרויקטים הייחודיים
  const uniqueProjects = [...new Set(tables.map(t => t.project_name || 'כללי'))];
  
  // בחירת פרויקט ברירת מחדל בטעינה
  useEffect(() => {
    if (uniqueProjects.length > 0 && !uniqueProjects.includes(activeProject)) {
      setActiveProject(uniqueProjects[0]);
    }
  ...
  // 1. קודם מסננים את כל הטבלאות ששייכות לפרויקט הנוכחי
  const projectTables = tables.filter(t => (t.project_name || 'כללי') === activeProject);

  // 2. שולפים את המערכות הייחודיות בתוך הפרויקט הזה
  const uniqueSystems = [...new Set(projectTables.map(t => t.system_name || 'לא הוגדר'))];

  // איפוס בחירת המערכת ל"הכל" בכל פעם שהמשתמש מחליף פרויקט
  useEffect(() => {
    setActiveSystem('הכל');
  }, [activeProject]);

  // 3. מסננים סופית לפי המערכת שנבחרה (או מציגים את כל מערכות הפרויקט אם נבחר "הכל")
  const finalDisplayedTables = activeSystem === 'הכל' 
    ? projectTables 
    : projectTables.filter(t => (t.system_name || 'לא הוגדר') === activeSystem);

  // קביעת טקסט התצוגה של המערכת בכרטיסיית המידע
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: '#f8f9fa' }}>
      <AppBar position="static" color="inherit" elevation={0} sx={{ borderBottom: '1px solid #dadce0', bgcolor: '#fff' }}>
        <Toolbar>
          ...
          {/* אייקוני סיכום ברמת ה-DS */}
          <Box sx={{ display: 'flex', gap: 0.5, mx: 2 }}>
            {hasFinancial && (
              <Tooltip title="מכיל נתונים פיננסיים" arrow>
                <AttachMoney sx={{ color: '#1e8e3e' }} />
              </Tooltip>
            )}
            {hasGeographical && (
              <Tooltip title="מכיל נתונים גאוגרפיים" arrow>
                <LocationOn sx={{ color: '#1a73e8' }} />
              </Tooltip>
            )}
            {hasSensitive && (
              <Tooltip title="מכיל נתונים רגישים" arrow>
                <Security sx={{ color: '#d93025' }} />
              </Tooltip>
            )}
          </Box>
       ...
       {/* Tab 0 — Tables */}
        {tab === 0 && (
          loadingTables
            ...
            : <Box>
                {/* לשוניות פנימיות עבור הפרויקטים (מוצגות רק אם יש יותר מפרויקט אחד - כמו ב MSSQL_mss_production) */}
