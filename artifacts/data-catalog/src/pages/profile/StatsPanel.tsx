...
const StatRow: React.FC<StatRowProps> = ({ label, value }) => (
  <Box display="flex" justifyContent="space-between" alignItems="center" py={0.65}>
    <Typography variant="body2" color="text.secondary">
      {label}
    </Typography>
    <Typography variant="body2" fontWeight={600} color="text.primary" dir="ltr">
      {value}
    </Typography>
  </Box>
);
...
const StatDivider = () => (
  <Box sx={{ height: '1px', bgcolor: '#dadce0', my: 0.75 }} />
);
...
// Falls back to an empty-state message when none apply.
  ...
  if (datetime_stats) {
    return (
      <Box>
        ...
        />
        {datetime_stats.format && <StatRow label="פורמט" value={datetime_stats.format} />}
      </Box>
    );
  }
