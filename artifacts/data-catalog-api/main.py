...
async def list_datasets(
    ...
    refresh:  bool          = Query(False, description="Bypass backend cache and fetch fresh data from GCP"),
):
    require_bq()
    try:
        # 1. שליפת רשימת ה-Datasets מ-BigQuery (עם תמיכה ב-refresh)
        datasets = await bq_service.list_datasets(refresh=refresh)

        # 2. שליפת התיאורים מ-Dataplex בצורה מקבילית
        if dataplex_service and dataplex_service.is_available:
            tasks = [dataplex_service.get_dataset_aspects(ds.dataset_id, refresh=refresh) for ds in datasets]
...
async def get_dataset(
    ...
    require_bq()
    try:
        ds = await bq_service.get_dataset(dataset_id, refresh=refresh)
        if not ds:
            raise HTTPException(status_code=404, detail=f"Dataset '{dataset_id}' not found")

        if dataplex_service and dataplex_service.is_available:
            aspects = await dataplex_service.get_dataset_aspects(dataset_id, refresh=refresh)
            if aspects.get("description_en"):
                ds.description = aspects.get("description_en")
            ds.description_he = aspects.get("description_he")
...
async def list_tables(
    ...
    require_bq()
    try:
        tables = await bq_service.list_tables(dataset_id, refresh=refresh)

        if dataplex_service and dataplex_service.is_available:
            dataset_aspects = await dataplex_service.get_dataset_aspects(dataset_id, refresh=refresh)
            ...
            if dataset_aspects.get("has_custom_aspects"):
                ...
                tasks = [dataplex_service.get_table_aspects(dataset_id, t.table_id, refresh=refresh) for t in tables]
...
async def list_relationships(
    ...
    require_bq()
    try:
        rels = await bq_service.list_relationships(dataset_id, min_confidence, refresh=refresh)
